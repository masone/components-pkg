...
| `SimpleGrid` | Chakra primitive + compatibility adapter | All `ChakraSimpleGridProps`, plus `spacing`, `spacingX`, `spacingY` | Complete Chakra surface, plus legacy aliases | Keep the complete public Chakra surface. Legacy-alias policy is decided separately in the adapter decision |
...
The selected public interfaces contain no Chakra `Pick` types: broad Chakra
primitives use their complete Chakra prop types, while Center expresses its
unchanged two-prop contract explicitly.

## Implementation consistency

The public contract selected here must be the canonical contract used inside the
package. Internal imports of `Stack` and `SimpleGrid` must not retain a narrower
`Pick` type or separate prop-normalisation behavior.

Today, package-root Stack accepts the complete Chakra surface plus `spacing`,
while `@/src/components/stack` exposes a narrower type. Package-root SimpleGrid
accepts the complete Chakra surface and current spacing aliases, while its inner
component has a narrower type even though the adapter forwards broad props
through it. For example, `Box as={Stack}` currently permits `h="full"` even
though the direct inner Stack type does not.

The adapter decision may retain aliases, reduce mappings, or remove adapters.
Whichever public contract is chosen, its type and behavior must have one
canonical implementation. This is an implementation requirement of this
decision, not a separate public-interface expansion.
